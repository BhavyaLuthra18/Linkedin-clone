...
const Main = (props) => {
  ...
  useEffect(() => {
    if (props.articles) {
      const articleIds = props.articles.map((article, index) => index);
      setIds(articleIds);
    }
  }, [props.articles]);

  const likeHandler = (event, key, id) => {
    event.preventDefault();
  
    if (props.articles && key >= 0 && key < props.articles.length) {
      const article = props.articles[key];
  
      if (article && article.likes) {
        let currentLikes = article.likes.count;
        let whoLiked = Array.isArray(article.likes.whoLiked) ? [...article.likes.whoLiked] : [];
        ...
        console.error("Likes property is undefined or missing");
      }
    } else {
      console.error("Invalid article index or missing articles property");
    }
  };
            ...
            {props.articles.length > 0 &&
              props.articles.map((article, key) => (
                <Article key={key}>
                  ...
                  <SocialCounts>
                  {article.likes && article.likes.count !== undefined && (
                    <li>
                      <button>
                        <img
...
position:relative;
border:none;
box-shadow: 0 0 0 1px rga(0 0 0/ 15%) ,  0 0 0 rgb(0 0 0 /20%);
`;
